...
export default class RestApi {
  ...
  query(options, callback) {
    ...
    const requestOptions = _.merge({
      ...
      qs: {
        start: options.start,
        pagesize: options.limit !== undefined ? Math.min(options.pageSize, options.limit) : options.pageSize
      }
    ...
    function loadRemainingPages(result) {
      ...
      if (options.limit !== undefined && isValidPagination && result.StartIndex + options.pageSize <= Math.min(options.limit, result.TotalResultCount)) {
        ...
        result.Results = results.slice(0, options.limit !== undefined ? options.limit : results.length);
