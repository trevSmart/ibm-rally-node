...
export default class RestApi {
  ...
  query(options, callback) {
    ...
    // Validate limit: must be a non-negative integer if provided
    if (options.limit !== undefined && options.limit < 0) {
      const error = new Error('Invalid limit: must be >= 0');
      callbackify(Promise.reject(error), callback);
      return Promise.reject(error);
    }

    // Handle special case: limit = 0 means no results wanted
    if (options.limit === 0) {
      const emptyResult = {
        Errors: [],
        Warnings: [],
    ...
    // Ensure optimalPageSize is never 0 or negative
    optimalPageSize = Math.max(1, optimalPageSize);
    ...
    function loadRemainingPages(result) {
      ...
      const hasMoreData = result.StartIndex + optimalPageSize <= result.TotalResultCount;
      ...
      if (isValidPagination && hasMoreData && withinLimit) {
        ...
        const nextPageOptions = {
          ...
          qs: Object.assign({}, requestOptions.qs, {
            start: result.StartIndex + optimalPageSize
          })
        };
  ...
  queryStream(options, onPageCallback, callback) {
    ...
    let serverTotalResultCount = null;

    async function processPages(result) {
      const pageResults = result.Results;

      // Store the server's total result count from the first page
      if (serverTotalResultCount === null && result.TotalResultCount) {
        serverTotalResultCount = result.TotalResultCount;
      }
      ...
      if (pageResults && pageResults.length > 0) {
        ...
        if (options.limit !== undefined) {
          ...
          if (remainingNeeded <= 0) {
            return { totalProcessed, completed: true, totalResultCount: serverTotalResultCount };
          }
          if (pageResults.length > remainingNeeded) {
        ...
        if (!continueProcessing) {
          return { totalProcessed, completed: false, totalResultCount: serverTotalResultCount };
        }
  ...
  queryBatch(options, batchSize, onBatchCallback, callback) {
    ...
    return this.queryStream(options, async (pageResults, pageInfo) => {
      // Store the server's total result count from the first page
      if (serverTotalResultCount === null && pageInfo.totalResultCount) {
        serverTotalResultCount = pageInfo.totalResultCount;
      }
      ...
      for (const result of pageResults) {
        ...
        if (currentBatch.length >= batchSize) {
          const batchInfo = {
            ...
            totalResultCount: serverTotalResultCount || pageInfo.totalResultCount
          };
      ...
      if (currentBatch.length > 0) {
        const batchInfo = {
          ...
          batchSize: currentBatch.length,
          totalProcessed: totalProcessed + currentBatch.length,
          totalResultCount: streamResult.totalResultCount || serverTotalResultCount
        };
